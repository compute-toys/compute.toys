...
import { getRainbowColor, Item } from '../../lib/util/theme';
        ...
        >
            <Timeline sx={{ alignItems: 'stretch', width: '100%', padding: '0 1rem' }}>
                {entryPoints.map((entryPoint, index) => (
                    ...
                    >
                        <TimelineSeparator>
                            <TimelineDot sx={{ backgroundColor: getRainbowColor(index, theme) }} />
                            {index < entryPoints.length - 1 ? <TimelineConnector /> : null}
                        </TimelineSeparator>
                        <TimelineContent color={getRainbowColor(index, theme)}>
                            <Box
                                style={{
                                    cursor: 'pointer'
                                }}
